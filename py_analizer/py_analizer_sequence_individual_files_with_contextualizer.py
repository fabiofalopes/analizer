...
FABRIC_MODEL = "mistral-nemo:latest"
#FABRIC_MODEL = "llama3.1:latest"
#FABRIC_MODEL = "qwen2:1.5b"
#FABRIC_MODEL = "phi3:14b"

TOKENIZER_MODEL = "bert-base-uncased"

TOKENS_PER_MINUTE_LIMIT = 6000
#CHUNK_SIZE = 4000  # Ajuste para o tamanho máximo de tokens por parte
CHUNK_SIZE = 600 #166666  
...
def process_content_for_pattern(content, chunk_size, encoded_tokens, pattern, model, output_filename_base, output_dir):
    base_output_filename = f"{pattern}_{model}_{output_filename_base}"
    output_file = get_unique_filename(output_dir, base_output_filename)
...
def main():
    ...
    # Create directory for contextual analysis
    context_analysis_dir = ROOT_FOLDER / "contextual_analysis"
    context_analysis_dir.mkdir(parents=True, exist_ok=True)
    ...
    for file_path in file_list:
        ...
        # Generate unique filename base for contextual analysis
        short_filename = Path(file_path).stem[:8]  # Using 8 characters of the filename
        hash_object = hashlib.md5(short_filename.encode())
        short_hash = hash_object.hexdigest()[:8]
        output_filename_base = f"contextual_analysis_{FABRIC_MODEL}_{short_hash}"

        # Perform contextual analysis
        process_content_for_pattern(
            content, 
            100000, # Numero de tokens muito maior que a analize para a analise de contexto
            encoded_tokens, 
            pattern="contextual_analysis",  # Always use 'contextual_analysis' pattern
            model=FABRIC_MODEL, 
            output_filename_base=output_filename_base,
            output_dir=context_analysis_dir
        )

        # Move the processed file to the contextual_analysis folder
        #output_file = get_unique_filename(context_analysis_dir, output_filename_base)
        #process_content_for_pattern(content, encoded_tokens, "contextual_analysis", FABRIC_MODEL, output_filename_base)

    # Abrir e adicicoanra para a lista de contextos
        
    if context_analysis_dir.is_dir():
        context_md_files = find_all_md_files(context_analysis_dir)
        context_file_list.extend(context_md_files)

    context = ""    

    # for file in context_file_list:
    #        with open(file, 'r', encoding='utf-8') as file: 
    #            context += file.read()


    # Continue with existing patterns
    for file_path, context_file in file_list, context_file_list:
        with open(file_path, 'r', encoding='utf-8') as file:
            content = file.read()
        with open(context_file, 'r', encoding='utf-8') as con_file:
            context = con_file.read()
            
        encoded_tokens = tokenizer.encode(context + content)
        ...
        for pattern in PATTERNS:
            ...
            process_content_for_pattern(context + content, CHUNK_SIZE, encoded_tokens, pattern, FABRIC_MODEL, output_filename_base, OUTPUT_ROOT_DIR)
